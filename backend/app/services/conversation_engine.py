...
class ConversationEngine:
    ...
    async def start_conversation(self, conversation_id: UUID, agent_ids: List[UUID]):
        ...
        participants = await self._load_participants(agent_ids)
        if not participants:
            return
    ...
    async def _load_participants(self, agent_ids: List[UUID]) -> List[Dict[str, Any]]:
        participants = []
        for aid in agent_ids:
            agent = await self.db.get(Agent, aid)
            if not agent:
                continue
    ...
    async def _get_history(self, conversation_id: UUID) -> List[Message]:
        result = await self.db.execute(
            select(Message)
    ...
    async def _save_message(
        self, conversation_id: UUID, agent_id: UUID, text: str, turn: int
    ) -> Message:
        msg = Message(
    ...
    async def generate_reply(
        ...
        system_prompt = self.agent_service.build_system_prompt(agent, profile)
    ...
    async def _run_free_mode(
        ...
        while self._running.get(str(conversation.id), False) and turn < max_turns:
            ...
            chat_messages = self._build_messages_for_agent(
                self.agent_service.build_system_prompt(agent, profile), history, agent.id
            )
            # Remove system from chat_messages since generate_reply adds it
            chat_messages = [m for m in chat_messages if m["role"] != "system"]

            reply = await self.generate_reply(agent, profile, chat_messages)
            await self._save_message(conversation.id, agent.id, reply, turn)
            turn += 1
    ...
    async def route_message(self, conversation_id: UUID, message: Message) -> Optional[UUID]:
        conversation = await self.db.get(Conversation, conversation_id)
        if not conversation:
            return None
        return await self.get_next_agent(conversation_id)

    async def get_next_agent(self, conversation_id: UUID) -> Optional[UUID]:
        conversation = await self.db.get(Conversation, conversation_id)
        if not conversation:
            return None
    ...
    async def build_context(self, conversation_id: UUID, agent_id: UUID) -> List[Dict[str, str]]:
