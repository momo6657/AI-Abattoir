...
class PokemonShowdownBattleAgent:
    ...
    def _plan_force_switch(self, request: ShowdownBattleRequest) -> ShowdownChoicePlan:
        ...
        choices: list[str] = []
        warnings: list[str] = []
        for forced in request.force_switch:
            if not forced:
                choices.append("pass")
                continue
    ...
    def _plan_moves(self, request: ShowdownBattleRequest, *, mode: str, allow_tera: bool) -> ShowdownChoicePlan:
        choices = [
            self._move_choice(active_request, mode=mode, allow_tera=allow_tera and index == 0)
        ...
        return ShowdownChoicePlan(
            ...
            request_id=request.request_id,
            decision_type="move",
            reason="Chose the highest-scored legal move for each active Pokemon.",
            needs_choice=True,
        )

    def _move_choice(self, active_request: dict[str, Any], *, mode: str, allow_tera: bool) -> str:
        ...
        if not legal_moves:
            return "default"
        move_slot, move = max(legal_moves, key=lambda item: self._score_move(item[1], mode))
        ...
        return f"move {move_slot}{target_part}{modifier_part}"

    def _score_move(self, move: dict[str, Any], mode: str) -> float:
        move_id = self.connector.to_id(move.get("id") or move.get("move"))
        if move_id in {"protect", "detect", "spikyshield", "kingsshield"}:
            return 35.0 if mode == "defensive" else 15.0
