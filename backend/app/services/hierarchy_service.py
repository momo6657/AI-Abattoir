...
class HierarchyService:
    ...
    async def send_command(
        ...
        # 验证权限
        has_authority = await self.check_authority(db, superior_id, subordinate_id)
        if not has_authority:
            raise ValueError("No authority to command this agent")

        superior = await db.get(Agent, superior_id)
        subordinate = await db.get(Agent, subordinate_id)
        ...
        conv_result = await db.execute(
            select(Conversation).where(
                Conversation.title == f"Command:{superior_id}:{subordinate_id}"
            )
        )
        conversation = conv_result.scalar_one_or_none()
        if not conversation:
            conversation = Conversation(
                title=f"Command:{superior_id}:{subordinate_id}",
                mode="free",
                config={"type": "hierarchy_command"},
            )
            db.add(conversation)
            await db.flush()
        ...
        message = Message(
            ...
            agent_id=superior_id,
            role="command",
            content={
                "text": command,
                "from": str(superior_id),
                "to": str(subordinate_id),
                "from_name": superior.name,
            },
            metadata_={"type": "hierarchy_command", "superior": str(superior_id)},
        )
        db.add(message)
        await db.commit()
        ...
        return {
            ...
            "from": {"id": str(superior_id), "name": superior.name},
            "to": {"id": str(subordinate_id), "name": subordinate.name},
            "command": command,
        }

    async def build_hierarchy_prompt(
        self, db: AsyncSession, agent_id: UUID, context_id: Optional[UUID] = None
    ) -> str:
        ...
        # 获取上级
        superior = await self.get_superior(db, agent_id)
        if superior:
            parts.append(
                f"你必须服从 {superior['agent_name']} 的指令。"
                f"当 {superior['agent_name']} 给你下达指令时，你需要执行。"
            )

        # 获取下级
        subordinates = await self.get_subordinates(db, agent_id)
        if subordinates:
            sub_names = ", ".join(s["agent_name"] for s in subordinates)
            parts.append(f"你有权指挥以下 agent：{sub_names}。")

        if context_id:
            parts.append(f"当前上下文 ID：{context_id}")
