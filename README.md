...
- Showdown 多格式计划预览：`training-program/plan` 可在执行前审查格式优先级、样本覆盖、Mastery 下限和可启动的无密码 program 请求
...
| POST | `/api/pokemon/showdown/training-program/plan` | 预览多格式训练 curriculum，返回格式优先级、样本覆盖、Mastery 下限、建议和无密码 `program_request`，不创建会话或发送命令 |
