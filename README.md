...
- Showdown 多格式训练计划：`training-program` 会按 curriculum 轮转多个格式运行训练循环，执行前应用逐格式 preflight gate，blocked 格式会跳过并进入 program health 人工审查，同时生成无密码 recovery program preset，先恢复 readiness/team audit 再继续训练
...
| POST | `/api/pokemon/showdown/training-program` | 按 `formats` 或自动 curriculum 选择多个 Showdown 格式，逐格式先执行 mission preflight gate，再运行 `training-loop`，汇总 evaluated/skipped 格式、program health、跨格式训练轮数、下一轮多格式 preset 和 blocked 格式的 recovery program preset |
