...
| POST | `/api/pokemon/showdown/training-chain` | 连续执行多轮 Showdown 自主任务，每轮先读取学习计划再执行 mission，并汇总 Mastery 分数、停止原因和未完成训练任务恢复队列；无手动动作白名单时，下一轮会自动应用上一轮 `recovery.actions` |
