...
- Showdown 多格式训练计划：`training-program` 会按 curriculum 轮转多个格式运行训练循环，执行前应用逐格式 preflight gate，blocked 格式会跳过并进入 program health 人工审查，最终生成无密码下一轮 preset
...
| POST | `/api/pokemon/showdown/training-program` | 按 `formats` 或自动 curriculum 选择多个 Showdown 格式，逐格式先执行 mission preflight gate，再运行 `training-loop`，汇总 evaluated/skipped 格式、program health、跨格式训练轮数和下一轮多格式 preset |
