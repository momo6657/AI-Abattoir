...
| POST | `/api/pokemon/showdown/training-chain` | 连续执行多轮 Showdown 自主任务，每轮先读取学习计划再执行 mission，并汇总 Mastery 分数、停止原因、未完成训练任务恢复队列和 `recovery_effectiveness`；无手动动作白名单时，下一轮会自动应用上一轮 `recovery.actions`，新链默认会通过 `resume_recovery=true` 继承历史未完成恢复动作 |
